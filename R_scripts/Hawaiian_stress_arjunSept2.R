...
# Contingency Tables - Arjun 

predicted_probs <- predict(model2, type = "response")
predicted_outcomes <- ifelse(predicted_probs > 0.5, 1, 0)

actual_outcomes <- dat_2syl$syllable_number[!is.na(dat_2syl$intensity) & !is.na(dat_2syl$Moras)]
contingency_table <- table(Predicted = predicted_outcomes, Actual = actual_outcomes)
...
coefficients_table <- cbind(Estimate = summary_model$coefficients, 
                            Std_Error = summary_model$standard.errors, 
                            Z_value = z_values, 
                            P_value = p_values)

# Calculate the effects
effect_list <- allEffects(model, partial.residuals = TRUE)


plot_with_pvalues <- function(effect, p_values, title = NULL) {
  p <- plot(effect, multiline = TRUE, rescale.axis = FALSE, residuals.pch = 15, main = title)
  

  for (i in 1:length(p$y.levels)) {
    text(x = max(p$fit[[i]]$x), 
         y = max(p$fit[[i]]$y), 
         labels = paste("p =", formatC(p_values[i], format = "e", digits = 2)),
...
# Apply the function to each effect plot and corresponding p-values
effect_plots <- map2(effect_list, as.data.frame(coefficients_table)$P_value, plot_with_pvalues)
